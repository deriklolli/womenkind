  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (res.status === 401) {
          router.replace('/signup?next=/get-started')
          return
        }
      } catch {
        router.replace('/signup?next=/get-started')
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({ addMembership }),
      })
        ...
        >
          ...
          <div className="text-center mt-6 mb-14">
            <h1 className="font-serif font-normal text-3xl md:text-4xl text-aubergine mb-4 tracking-tight leading-tight">
              Your menopause care starts here
            </h1>
            <p className="text-base text-beige/70 font-sans leading-relaxed max-w-lg mx-auto">
              Choose your care plan below. After checkout you&apos;ll complete a brief health assessment so your provider can personalize your treatment.
            </p>
          </div>
          ...
          <div className="bg-white border border-violet rounded-[20px] overflow-hidden flex flex-col shadow-sm">
            ...
            <div className="px-6 py-5 space-y-3 flex-1">
              {[
                { text: 'Comprehensive health assessment review' },
                { text: 'Personalized treatment pathway' },
                { text: 'Your Future Health Blueprint' },
                { text: 'Initial provider consultation (telehealth)' },
              ].map((item) => (
          ...
          <div className="mt-10 flex flex-col items-center">
            <div className="text-center mb-4">
              ...
              <div className="mt-1">
                <span className="text-3xl font-sans font-bold text-aubergine">$650</span>
              </div>
              {addMembership && (
