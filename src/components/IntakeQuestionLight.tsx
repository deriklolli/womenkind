...
export default function IntakeQuestionLight({
  question,
  value,
  onChange,
  onNext,
  ...
  return (
    ...
    >
      ...
      {!question.req && (
        <div className="mb-6">
          <span className="text-xs font-sans text-beige/40 bg-aubergine/5 px-2.5 py-0.5 rounded-full">
            Optional
          </span>
        </div>
      )}
      ...
      {/* Label */}
      <h2 className="font-sans font-semibold text-2xl md:text-3xl text-aubergine leading-snug mb-3 tracking-tight">
        {question.label}
      </h2>

      {/* Sub-label */}
      {question.sub && (
        <p className="text-sm text-beige/60 font-sans mb-6 leading-relaxed">{question.sub}</p>
      )}
      ...
      <div className="mt-8">
        {(question.type === 'text' || question.type === 'number') && (
          <input
            ...
            className="w-full bg-transparent border-b-2 border-aubergine/15 focus:border-violet py-4 text-aubergine text-lg
                       placeholder:text-beige/30 font-sans outline-none transition-colors duration-300"
          />
        )}
        ...
        {question.type === 'textarea' && (
          <textarea
            ...
            placeholder={question.ph}
            rows={4}
            className="w-full bg-white border border-aubergine/10 focus:border-violet rounded-2xl p-5 text-aubergine text-base
                       placeholder:text-beige/30 font-sans outline-none transition-colors duration-300 resize-none shadow-sm"
          />
        )}
            ...
            {question.opts.map((opt) => (
              <button
                ...
                className={`w-full text-left px-5 py-4 rounded-2xl font-sans text-base transition-all duration-200
                  ${
                    value === opt
                      ? 'bg-violet/10 border-2 border-violet text-aubergine'
                      : 'bg-white border-2 border-aubergine/10 text-beige hover:bg-aubergine/[0.03] hover:border-aubergine/20 shadow-sm'
                  }`}
              >
                <span className="flex items-center gap-3">
                  <span
                    className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all duration-200
                    ${value === opt ? 'border-violet bg-violet' : 'border-aubergine/25'}`}
                  >
                    {value === opt && (
                ...
                >
                  <span className="flex items-center gap-3">
                    <span
                      ...
                      ${selected ? 'border-violet bg-violet' : 'border-aubergine/25'}`}
                    >
                      {selected && (
      ...
      <div className="flex items-center justify-between mt-10">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-3 rounded-full font-sans text-sm font-medium transition-all duration-200
            ${isFirst ? 'opacity-0 pointer-events-none' : 'text-beige/60 hover:text-aubergine hover:bg-aubergine/5'}`}
        >
        ...
        {question.type === 'single' && canProceed && (
          <button
            onClick={onNext}
            className="flex items-center gap-2 px-5 py-3 rounded-full font-sans text-sm font-medium text-beige/40 hover:text-aubergine hover:bg-aubergine/5 transition-all duration-200"
          >
            {isLast ? 'Submit' : 'Next'}
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
      ...
      {question.type !== 'single' && question.type !== 'multi' && (
        <p className="text-center text-xs text-beige/30 mt-6 font-sans">
          Press <kbd className="px-1.5 py-0.5 bg-aubergine/5 rounded text-beige/50 text-[10px]">Enter</kbd> to continue
        </p>
      )}
    </div>
  )
