...
export default function AppointmentsList({ providerId }: Props) {
  ...
  const [visitPrepApt, setVisitPrepApt] = useState<{ id: string; patientName: string } | null>(null)
  ...
  return (
    <div>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        {apt.status === 'confirmed' && (
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => setVisitPrepApt({ id: apt.id, patientName })}
      ...
      {/* Visit Prep slide-over panel */}
      {visitPrepApt && (
        <VisitPrepPanel
          appointmentId={visitPrepApt.id}
          patientName={visitPrepApt.patientName}
          onClose={() => setVisitPrepApt(null)}
        />
      )}
    </div>
  )
