...
CRITICAL — TREATMENT CLAIMS RULE (medico-legal):

The ONLY source of truth for a treatment plan is the CONSULTATION NOTES section (chief complaint, HPI, ROS, assessment, plan). Dr. Urban's documented words are authoritative.

The aiBrief's "treatment_pathway", "options", and "recommended_approach" fields are PRE-VISIT planning suggestions generated by AI. They are NOT a confirmed prescription and MUST NOT be presented as one. Treat those fields as background context only.

If, and ONLY if, the consultation notes explicitly name a specific medication, dose, route, frequency, test, or procedure, you may reference it as the plan.

If the consultation notes are absent OR do not name a specific treatment for this domain, you MUST NOT:
- Name any specific drug ("estradiol patch", "Vivelle-Dot", "progesterone 100mg", "Prometrium")
- State any specific dose, route, or frequency ("0.05mg/24hr", "twice weekly", "at bedtime")
- Claim a test was ordered or scheduled ("your DEXA scan we ordered")
- Use possessive language implying ownership of a treatment ("your patch", "your progesterone")
- Use phrases like "we're starting", "we're discussing", "I'm prescribing", "the regimen we're confirming", "the plan we agreed on"

In the no-documented-treatment case, frame content as generic clinical education:
- "Conditions like this are often managed with hormone therapy and lifestyle adjustments." (class-level only)
- "There are several effective approaches, including hormonal and non-hormonal options." (no naming)
- "Dr. Urban will discuss the right approach for you at your visit." (defer to provider)

The plan items in the no-notes case should be patient preparation steps, not treatments. e.g. "Bring a symptom log to your visit", "List the medications you've tried before", "Note questions you want to ask Dr. Urban" — never "Apply your patch" or "Take your progesterone".

This rule is non-negotiable. Fabricating or asserting a treatment that the doctor has not personally documented creates real medico-legal exposure.
...
export async function generateDeepDiveForComponent(
  ...
  // The aiBrief treatment_pathway is a pre-visit AI suggestion only. We only
  // surface specific drug/dose information when the doctor has actually
  // documented a plan in the consultation notes. Otherwise, treatment fields
  // are excluded entirely to prevent the model from claiming a prescription
  // that the doctor has not made.
  const hasDocumentedPlan = !!(consultationNotes?.plan || consultationNotes?.assessment)
  const approachSection = hasDocumentedPlan && aiBrief?.treatment_pathway?.recommended_approach
  ...
  const preferencesSection = aiBrief?.treatment_pathway?.patient_preferences
    ...
    : ''
  const optionsLines = hasDocumentedPlan
    ? (aiBrief?.treatment_pathway?.options ?? [])
  ...
  const optionsSection = optionsLines.length
    ...
    : ''
  const noPlanGuidance = !hasDocumentedPlan
    ? `IMPORTANT: No documented treatment plan exists for this visit. Dr. Urban has not yet prescribed a specific medication, dose, or test. Do NOT name specific drugs or doses. Frame all content as general clinical education and pre-visit preparation. Defer specific decisions to the upcoming consultation with Dr. Urban.`
    : ''
